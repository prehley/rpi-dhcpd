...
The following was copied from the networkboot/docker-dhcpd project.  The project name was changed to prehley/rpi-dhcpd
...
============
 ...
 2. Run `./build` to create the local docker image `prehley/rpi-dhcpd`.
...
on.
 ...
 2. Create `data/dhcpd.conf` with a subnet clause for the specified
    ...
    `docker run -it --rm prehley/rpi-dhcpd man dhcpd.conf` for a description
    of the configuration file syntax.
 3. Run `docker run -it --rm --net=host -v "$(pwd)/data":/data prehley/rpi-dhcpd eth0`.
